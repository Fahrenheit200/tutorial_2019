...
{
	uint8 i;
	uint16 chrIndex=(chr-32)*6;
	oledSetPos(x, y);
	for(i=0;i<6;i++)
	oldeWrite(smallChar[chrIndex+i], OLED_DATA);
} 
void oledBigChar(uint8 x,uint8 y,uint8 chr)
{
	uint16 chrIndex=(chr-32)*16;
	uint8 i;
	oledSetPos(x, y);
	for (i =0;i<8;i++)
	oldeWrite(bigChar[chrIndex+i], OLED_DATA);
	oledSetPos(x, y+1);
	for (i=0;i<8;i++)
	oldeWrite(bigChar[chrIndex+i+8], OLED_DATA);
}
